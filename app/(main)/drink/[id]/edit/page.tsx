...
const Edit = () => {
    ...
    const checkUser = async () => {
        ...
        if (user === null) {
            toast.warning("Must create an account to create/edit drinks!")
            router.push("/")
            return;
        }
        else {
        ...
        if (data === null) {
            toast.warning("A drink with that id does not exist!")
            router.push("/")
            return
        }

        const drink: Drink = data
        if (drink.created_by !== user.id) {
            toast.warning("Your are not the creator of this Drink!")
            router.back();
        }
        ...
        const ingredientRows = drink.drink_ingredients.map((ingredient, _) => {
        ...
        toast.success("Success!", {
            description: "Your drink was loaded!"
        })
        setLoading(false);
    }
    ...
    const handleDrinkForm = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        const edited = {
            ...
            alcoholic,
            glass,
            image_url: image === null ? drink!.image_url : image_path,
            instructions,
